...
Route::group([
    ...
    Route::prefix('/trips')->group(function () use ($namespace) {
        $controller = $namespace.'\TripsController';
        $tripcontroller = $namespace.'\TourController';
     
             
        Route::get('/', "$controller@index")->name('trip_index');
        Route::get('/alltrips', "$tripcontroller@index")->name('tour_index');
        Route::get('/mytrips', "$tripcontroller@tour_myindex")->name('tour_myindex');
        Route::post('rvsp', "$tripcontroller@rvsp")->name('rvsp');
        Route::get('bookingdetail/{trip_id}',"$tripcontroller@bookingdetail")->name('bookingdetail');
        Route::post('rvspstore', "$tripcontroller@rvspstore")->name('rvspstore');
        Route::post('booking', "$tripcontroller@booking")->name('booking');
        
      Route::get('/{country}/{state}/{city}/{event}', "$tripcontroller@show") ->name('trips_show');
        ...
        Route::get('/api/get_trips', "$tripcontroller@events")->name('trips');
        Route::get('/api/getcategory', "$tripcontroller@getcategory")->name('mytrips_categories');
          });
          ...
          Route::get('/', "$tripcontroller@alltouropreter")->name('all_opreter_detail');
