...
Route::prefix('/touroperator')->group(function () use ($namespace) {
    ...
    Route::get('detailingimage/{id}', "$tourcontroller@detailimages")->name('detailimage');
...
Route::group([
    ...
    Route::prefix('/allevents')->group(function () use ($namespace) {
        ...
        $mytripcontroller = $namespace.'\MyEventsController';
        Route::get('/', "$controller@index")->name('events_index');
        
        // Wildcard

        Route::get('/api/getticket_title', "$mytripcontroller@tripticket_title")->name('myticket_title');
        Route::get('/api/getevticket_title', "$mytripcontroller@eventticket_title")->name('evticket_title');
        Route::get('/api/getticket_currency', "$mytripcontroller@tripcurrencies")->name('myticket_currency');

        Route::get('/{event}', "$controller@show")->name('events_show'); 
        Route::get('/{event}/tag/{tag_title}', "$controller@tag")->name('events_tags'); 
        Route::post('rvsp', "$controller@rvsp")->name('rvsp'); 
        Route::post('rvspstore', "$controller@rvspstore")->name('rvspstore');  
    ...
    Route::prefix('/showtrip')->group(function () use ($namespace) {
    ...
    Route::prefix('/bookings')->group(function () use ($namespace)  {
      ...
      /* My Bookings (organizer) */
      Route::prefix('/tripbookings')->group(function () use ($namespace)  {
        $controller = $namespace.'\OBookingsController';
        
        Route::get('/', "$controller@trindex")->name('trbookings_index');  


          // API
          Route::get('/api/trip_organiser_bookings', "$controller@triporganiser_bookings")->name('trip_obookings_organiser_bookings');
    ...
    Route::prefix('/myevents')->group(function () use ($namespace) {
        ...
        Route::get('book', "$controller@book")->name('bookingsmy_index');
        ...
        Route::post('/api/store_payment', "$controller@storeevent_payment")->name('myevent_store_payment');
        ...
        Route::get('/api/countriesmy', "$controller@countriesmy")->name('eventsmy_countries'); 
        ...
        Route::get('/api/add_discount', "$controller@add_discount")->name('add_discount'); 
        Route::get('/api/addsecond_discount', "$controller@addsecond_discount")->name('addsecond_discount');
        Route::get('/api/mydiscountweek', "$controller@mydiscountweek")->name('mytrips_discountweek'); 
        Route::get('/api/mydiscountsecondweek', "$controller@mydiscountsecondweek")->name('mytrips_seconddiscountweek'); 
        Route::get('/api/mytrips_discount', "$controller@mydiscount")->name('mytrips_discount'); 
        Route::get('/api/insertselect_discount', "$controller@insertselect_discount")->name('insertselect_discount');
        Route::get('/api/insert_discount', "$controller@insert_discount")->name('insert_discount');
        Route::get('/api/get_discount', "$controller@get_discount")->name('get_discount'); 
    ...
    Route::prefix('/mytour')->group(function () use ($namespace)  {
        ...
        Route::post('/api/store_ticket', "$tourcontroller@storetrip_ticket")->name('mytrip_store_ticket');
        ...
        Route::post('/api/store_payment', "$tourcontroller@storetrip_payment")->name('mytrip_store_payment');
    });

    Route::prefix('/myvisiter')->group(function () use ($namespace)  {
        $controller = $namespace.'\MyEventsController';
        Route::get('/', "$controller@visitindex")->name('visit_index');
    });
    /* Notification */
            ...
            if(\Auth::user()->hasRole('organiser')) {
                ...
                if($n_type == "bookings" || $n_type == "cancel" )
                    return redirect()->route('eventmie.obookings_index');  
                        ...
                        // create booking notification
                        if($n_type == "bookings" || $n_type == "cancel" )
                            return redirect()->route('eventmie.bookingsmy_index');    
